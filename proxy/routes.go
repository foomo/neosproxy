...
func (p *Proxy) setupRoutes() {
	...
	p.router.HandleFunc(routeContentServerExport, p.streamCachedSitemap).Queries("workspace", "{workspace}")
	...
	p.router.Use(p.middlewareWorkspace)
	...
	neosproxyRouter.Use(p.middlewareWorkspace)
